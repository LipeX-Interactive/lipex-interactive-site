(() => {
  ...
  function normalizeConfig(input) {
    ...
    // Keep the public storefront aligned with the current Paddle catalog during
    // the Sandbox rollout, even if an older published Site Editor snapshot still
    // contains the previous USD amounts. The admin can be updated later to make
    // these values fully editable again.
    config.catalog_items = config.catalog_items.map((item) => {
      if (item?.product_slug === "messi-vs-ronaldo") return { ...item, price_usd: 28.99 };
      if (item?.product_slug === "android-vs-iphone") return { ...item, price_usd: 29.99 };
      return item;
    });
